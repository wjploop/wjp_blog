...
一般的应用中，ViewPager 会搭配 FragmentPagerAdapter使用，而在本Launcher中，是直接继承了PagerAdaper （FragmentPagerAdapter的父类）自定了一个Adapter，自己管理每一个页的GridLayout，而非使用Fragment承载。为何？其原因我不太确定，我猜这是以前大佬的对代码洁癖（褒义）的追求吧，能用基础的类完成，绝不会引用庞大的工具包。dｄ
